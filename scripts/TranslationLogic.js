...
async function loadDictionary() {
    ...
    const glossaryJournal = game.journal.find(j => j.name === "AI Glossary");
    if (glossaryJournal) {
...
// Helper to inject markers into Russian text (Reverse Lookup)
// dictionary is English -> Russian
export async function injectGlossaryMarkers(docData) {
    ...
    const allTranslatedTerms = Object.values(dictionary);
    const uniqueTerms = [...new Set(allTranslatedTerms)].filter(t => t && t.length > 2);
...
export async function processUpdate(doc, rawText, processingMode = 'translate', selectedPageIds = null) {
    ...
    if (jsonMatches.length > 0) {
        ...
        if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
            try {
                ...
                if (json.name === "AI Glossary") glossaryJournalJson = json;
    ...
    try {
        ...
        if (glossaryJournalJson) {
            const existing = game.journal.find(j => j.name === "AI Glossary");
            if (existing) {
                ...
                ui.notifications.info(loc('InfoGlossaryCreated') || "New Journal 'AI Glossary' created!");
            }
            ...
            if (processingMode === 'grammar') {
                ...
                const recoverContext = (targetId, jsonData, referenceData) => {
                    try {
                        ...
                        cleanGap = cleanGap.replace(/^[^\wа-яА-ЯёЁ("]+/, "")
                            .replace(/[^\wа-яА-ЯёЁ).!?"']+$/, "");


                        if (cleanGap.length < 2) return "[DELETED]";

                        if (cleanGap.length > 250) cleanGap = cleanGap.substring(0, 250) + "...";

                        console.log(`[DH Translator] Gap Found between ${startMarker} and ${endMarker}`);
                        return `[... ${cleanGap} ...] (Between ${startMarker} & ${endMarker})`;
                ...
                console.log("DH Translator | Updated Scopes identified:", updatedScopes);
    ...
    try {
        ...
        if ((doc.documentName === "Actor" || doc.documentName === "Item") && jsonData.items && Array.isArray(jsonData.items)) {
            jsonData.items = jsonData.items.map(newItem => {
                if (!newItem._id && doc.items) { console.warn(`DH Translator | Safety: Item without ID skipped.`); return null; }
                if (doc.items) {
        ...
        if (jsonData.type && jsonData.type !== doc.type) ui.notifications.warn(loc('WarnTypeChange') || `Warning: Type-Change!`);
...
export async function addToGlossary(newItems) {
    ...
    console.log(`DH Translator | Found ${allTerms.length} existing terms.`);
    ...
    if (addedCount > 0) {
        ...
        if (ulStartMatch && ulEndIndex > ulStartMatch.index) {
            ...
            if (content.trim().length === 0) {
                preContent = "<h1>Автоматический глоссарий</h1><p>Обратите внимание, что это автоматически переведённые термины. При ошибках проверяйте первоисточник.</p><hr>";
            } else {
